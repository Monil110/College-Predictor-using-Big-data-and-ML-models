...
def predict_admission(rank, institute, program, category, quota, pool, round_no=6, latest_year=2021):
    try:
        ...
        return {"error": f"Models not trained yet for {inst_type}. Run train.py first."}
        
    try:
        # Load historical features to get corresponding opening_rank, closing_rank, degree_short
        df = pd.read_csv("data/processed/features.csv")
    except Exception as e:
        return {"error": f"Failed to load features.csv: {e}"}
        
    # Filter dataset for exact matching program details
    temp = df[
    ...
    # We take the first matched historic row as reference for opening_rank and degree_short
    ...
    # Predict the expected closing rank
